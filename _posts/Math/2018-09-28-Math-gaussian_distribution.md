...
\\( \left(\begin{array}{cccc} \frac{1}{N} \frac{1}{N} \ldots \frac{1}{N} \end{array}\right) \begin{pmatrix} x\_1^{(1)} & x\_1^{(2)} & \ldots & x\_1^{(N)} \\\\ x\_2^{(1)} & x\_2^{(2)} & \ldots & x\_2^{(N)} \\\\ \vdots  & \vdots & \vdots & \vdots \\\\ x\_D^{(1)} & x\_D^{(2)} & \ldots & x\_D^{(N)} \end{pmatrix} = \frac{x^{(1)}}{N} \\)
