...
1. $x = (\begin{array}{c} x\_1 & \ldots & x\_n \end{array})^T \text{: } D \times 1 \text{행렬}$
2. $\mu = (\mu\_1 \ldots \mu\_n)^T \text{: D \times 1 행렬}$ ($x$ 행렬로 부터 $\mu$ 행렬을 구하는 방법)
3. $\Sigma \text{: D \times D 공분산 행렬}$
4. $\Sigma^{-1} \text{: D \times D 공분산 행렬의 역행렬}$
