...
\\( \begin{align} \sum\_{k=1}^{n}(X\_k - \bar{X})^2 = \sum\_{k=1}^{n}\{(X\_k - \mu) + (\mu - \bar{X})\}^2 \\ & = \sum\_{k=1}^{n}(X\_k - \mu)^2 + 2(\mu - \bar{X})n(\bar{X} - \mu) + n(m - \bar{X})^2 \\ & = \sum\_{k=1}{n}(X\_k - \mu)^2 + n(\bar{X} - \mu)^2  \\) 
